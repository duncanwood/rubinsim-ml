...
from rubin_sim.maf.utils import m52snr, load_inst_zeropoints
...
def poisson_noise_from_mag(m, m0, exp_time=30):
    """
    Calculates photon shot noise in mags on a source with magnitude m and 
    zero-point magnitude m0.
    ...
    m : `float`
        Source magnitude
    m0 : `float`
        Zero-point magnitude of the system
    exp_time : `float`
        Exposure time in seconds

    Returns
    -------
    Poisson noise in mags (float)
    """
    n_photons = exp_time*np.power(10, (m0-m)/2.5)
    n_photons_err = np.sqrt(n_photons)
    return (2.5*np.log10(1+1/n_photons_err))

def poisson_snr_from_mag(m, m0, exp_time=30):
    """
    Calculates photon shot noise in mags on a source with magnitude m and 
    zero-point magnitude m0.
...
def add_snr(snrs): 
    return np.power(sum([1/snr**2 for snr in snrs]), -1/2)
...
class MicrolensingMetric(metrics.BaseMetric):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.detect = detect
        self.mags = mag_dict
        self.zp_inst, self.k_atm = load_inst_zeropoints()
    ...
    def run(self, data_slice, slice_point=None):
        ...
        snr = add_snr([m52snr(amplified_mags, data_slice[self.m5_col]),
                       poisson_snr_from_mag(amplified_mags, np.array([self.zp_inst[filt] for filt in data_slice[self.filter_col]]))])
